...
function Checkout() {
  const { username, phonenumber, address, city, country,setUsername, setPhonenumber, setAddress, setCity, setCountry  } =
...
const fetchData = () => {
  ...
  const handleOrder = () => {
    ...
    if (products.length > 0) {
        api.post("http://localhost:5000/order", orderData )
        .then((response) => {
        ...
        <div className="col col-6 infor-checkout">
          ...
          <div className="infor-checkout-box">
            {checkoutData.Items.map((item) => (
              <div key={item._id}>
                <p>
                  <span className="infor-checkout-name">{item.name}</span>- (Số
                  lượng: {item.quantity} - Giá: {item.newPrice}đ)
                </p>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={() => handleOrder()}
            className="infor-checkout-button"
          >
            Đặt hàng
          </button>
        </div>
